...
public class BucketOfBlood extends ItemFluidContainer
{
	private int isFull;

	public BucketOfBlood(int id, int par2)
	{
		super(id);
		this.isFull = par2;
		MinecraftForge.EVENT_BUS.register(this);
		setTextureName("cellarium:bucketOfBlood");
		setUnlocalizedName("bucketOfBlood");
	...
	private ItemStack attemptFill( World world, MovingObjectPosition p )
	{
		...
		if ( id == Cellarium.blockBlood.blockID )
		{
			if ( world.getBlockMetadata( p.blockX, p.blockY, p.blockZ ) == 0 ) // Check that it is a source block
			{
				...
				return new ItemStack( Cellarium.bucketOfBlood ); // Return the filled bucked item here.
			}
		}
