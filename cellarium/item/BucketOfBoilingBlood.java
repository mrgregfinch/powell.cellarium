...
public class BucketOfBoilingBlood extends ItemFluidContainer
{
	private int isFull;

	public BucketOfBoilingBlood(int id, int par2)
	{
		super(id);
		this.isFull = par2;
	...
	public boolean tryPlaceContainedLiquid(World par1World, int par2, int par3, int par4)
	{
		...
		else
		{
			...
			{
				if (par1World.provider.isHellWorld && this.isFull == Block.lavaMoving.blockID)
				{
	...
	private ItemStack attemptFill( World world, MovingObjectPosition p )
	{
		...
		if ( id == Cellarium.blockBoilingBlood.blockID )
		{
			if ( world.getBlockMetadata( p.blockX, p.blockY, p.blockZ ) == 0 ) // Check that it is a source block
			{
				...
				return new ItemStack( Cellarium.bucketOfBoilingBlood ); // Return the filled bucked item here.
			}
		}
